    ...
    if (answer.operator === 'Add A New Student') {
        let addStudent = await inquirer.prompt([
            ...
            },
            {
                ...
                validate: (input: string) => /^\d{5}$/.test(input) ? true : 'Please enter a 5-digit ID.',
            },
            {
                type: 'input',
                name: 'fees',
                message: (chalk.yellow('Enter student fees:')),
                validate: (input: string) => /^\d+(\.\d{1,2})?$/.test(input) ? true : 'Please enter a valid amount.',
            },
            {
                type: 'checkbox',
                name: 'courseEnroll',
        ...
        students.push(addStudent as Student);
